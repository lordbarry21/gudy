      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute top-5 right-5 p-2 rounded-xl text-text-muted hover:text-text-primary hover:bg-surface-elevated transition-colors cursor-pointer z-20"
          ...
          <div className="relative flex bg-surface-elevated p-1 rounded-xl mb-4 shrink-0 text-xs font-semibold">
            <button
              type="button"
              onClick={() => setActiveTab('overview')}
              className={`relative flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg transition-colors cursor-pointer z-10 ${
                activeTab === 'overview'
                  ? 'text-text-primary'
          ...
          {/* Scrollable Content Body with Smooth Sliding Transitions */}
            ...
            <AnimatePresence mode="wait" initial={false}>
              {activeTab === 'overview' ? (
                ...
                >
                  ...
                  {/* 5. Smooth Transition Card to Streak Freeze */}
                  <div className="p-3.5 rounded-2xl bg-sky-500/10 flex items-center justify-between gap-3 shadow-inner">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-text-primary text-xs truncate">
                          {t.home.streakFreezeTransitionTitle}
                        </p>
                        <p className="text-[11px] text-text-muted truncate">
                          {t.home.streakFreezeTransitionDesc}
                        </p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setActiveTab('freeze')}
                      className="shrink-0 inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-sky-500/20 hover:bg-sky-500/30 text-sky-400 font-bold text-xs transition-colors cursor-pointer"
                    >
                      <span>{t.home.seeFreezeDetails}</span>
                ...
                >
                  ...
                  {/* Back to Daily Streak Tab Shortcut */}
                  <button
                    type="button"
                    onClick={() => setActiveTab('overview')}
                    className="w-full py-2 rounded-xl bg-surface-elevated hover:bg-surface text-text-muted hover:text-text-primary text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
